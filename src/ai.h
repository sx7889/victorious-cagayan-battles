...
namespace ai
{
    ...
    struct aiinfo
    {
        ...
        vector<int> route;
        vec target, spot;
        int enemy, enemyseen, enemymillis, triggermillis, weappref, prevnodes[NUMPREVNODES], targnode, targlast, targtime, targseq,
            lastrun, lasthunt, lastaction, lastcheck, jumpseed, jumprand, blocktime, huntseq, blockseq, lastaimrnd, guardtime;
        ...
        void clearsetup()
        {
            ...
            lastaction = lasthunt = lastcheck = enemyseen = enemymillis = blocktime = huntseq = blockseq = targtime = targseq = lastaimrnd = guardtime = 0;
            triggermillis = 0;
    ...
    extern void init(gameent *d, int at, int on, int sk, int bn, int pm, int col, const char *name, int team, int);
    extern void update();
    extern void avoid();
