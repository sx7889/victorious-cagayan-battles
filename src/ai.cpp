...
namespace ai
{
    ...
    VAR(aidebug, 0, 0, 6);
    VAR(aiforcegun, -1, -1, 3);
    VAR( aienabled, 0, 1, 1 );

    ICOMMAND(addbot, "sii", (char *s, int *team, int *cls), addmsg( N_ADDBOT, "riii", *s ? clamp(parseint(s), 1, 101) : -1, *team, *cls ) );
    ...
    {
        return max(int(skills[atk].exprad), 2);
    }

    float attackmaxdist(int atk)
    {
        return skills[atk].range + 5;
    }
    ...
    bool targetable(gameent *d, gameent *e)
    {
        if( d == e
        || !canmove(d)
        || e->cloakmode ) return false;
        bool ret = e->state == CS_ALIVE && !isteam(d->team, e->team);
        return ret;
    }
    ...
    bool canshoot(gameent *d, int atk, gameent *e)
    {
        if(attackrange(d, atk, e->o.squaredist(d->o)) && targetable(d, e))
            return lastmillis - d->lastaction >= d->gunwait;
        return false;
    }
    ...
    { // add margins of error
        ...
        {
            float skew = clamp(float(lastmillis-d->ai->enemymillis)/float((d->skill*skills[atk].attackdelay/200.f)), 0.f, skills[atk].projspeed ? 0.25f : 1e16f),
    ...
    bool hasrange( gameent *d, gameent *e, int skill )
    {
        if(!e) return true;
        if(targetable(d, e))
        {
            ...
            if(attackrange(d, skill, dist)) return true;
        }
        return false;
    }
    ...
    void init(gameent *d, int at, int ocn, int sk, int bn, int pm, int col, const char *name, int team, int classid)
    {
        loadwaypoints();
    ...
    int isgoodammo(int gun) { return true; } //gun == GUN_PULSE || gun == GUN_RAIL; }
    ...
    void setup(gameent *d)
    {
        ...
        d->ai->weappref = -1;//classes[ rnd(NUMCLASSES) ].weaponid;
        ...
        if(d->state == CS_ALIVE)
        {
            switch(b.targtype)
            {
                ...
                case AI_T_PLAYER:
                {
                    ...
                    {
                        int atk = classes[ d->classid ].skill1;
                        float guard = SIGHTMIN, wander = skills[atk].range;
    ...
    // base
    struct leaf {
        virtual bool execute( gameent*, aistate & ) {};
    };

    struct sequence : leaf {
        vector<leaf*> childs;

        bool execute( gameent *d, aistate &b ) {
            loopv(childs) {
                leaf *child = childs[ i ];
                if( !child->execute( d, b ) ) {
                    return false;
                }
            }
    ...
    // impl
    struct btisenemyok : leaf {
        bool execute( gameent *d, aistate &b ) {
            if( !game::allowedtoattack( d ) ) {
    ...
    struct btlookatenemy : leaf {
        bool execute( gameent *d, aistate &b ) {
            ...
            if( insight || hasseen )
            {
                ...
                scaleyawpitch(d->yaw, d->pitch, yaw, pitch, 1, sskew);
                return true;
            }
    ...
    struct btcanshoot : sequence {
        bool execute( gameent *d, aistate &b ) {
            if( !game::canshoot( d ) ) {
                return true;
            }

            return sequence::execute( d, b );
        }
    };

    struct btcounterattack : leaf {
        bool execute( gameent *d, aistate &b ) {
            if( d->skill < 80 ) {
                return true;
            }
            ...
            gameent *e = getclient(d->ai->enemy);
            int guardskill = classes[ d->classid ].guardskill;

            if( e->nextskill != NO_SKILL
            && d->nextskill == NO_SKILL ) {
                const skillinfo &skill = skills[ e->nextskill ];
                bool canblock = skill.block == 0 && skill.canbeblocked;
                if( canblock ) {
                    d->nextskill = guardskill;
                    d->ai->triggermillis = lastmillis;
                    d->ai->lastaction = lastmillis;
                    return false;
                }
            }
            if( d->nextskill == guardskill ) {
                if( lastmillis - d->ai->triggermillis > 2000 ) {
                    d->nextskill = NO_SKILL;
                }
                return false;
            }
    ...
    struct btattack : leaf {
        bool execute( gameent *d, aistate &b ) {
            gameent *e = getclient(d->ai->enemy);
            int skillid1 = classes[ d->classid ].skill1;
            int skillid2 = classes[ d->classid ].skill2;
            int skillids[] = {
                skillid1,
                skillid2,
                NO_SKILL
            };
            int skillid = NO_SKILL;
            int skilldamage = 0;

            for( int i=0; skillids[i] != NO_SKILL; ++i ) {
                const skillinfo &skill = skills[ skillids[i] ];
                if( skill.damage > skilldamage ) {
                    skillid = skillids[i];
            ...
            const skillinfo &skill = skills[ skillid ];

            bool underchain = lastmillis - d->lastchaintime < CHAIN_COOLDOWN;
            int waittime = underchain ? 100 : skill.attackdelay;
            bool heldtrigger = lastmillis - d->ai->triggermillis > waittime;

            vec dp = d->headpos();
            vec ep = getaimpos(d, e);
            float dist = dp.squaredist( ep );
            int range = skill.range;
            bool closerange = dist < range * range;
            d->ai->dontmove = closerange;
    ...
    {
        if( !btinit ) {
            sequence *btcombat = new sequence;
            btcombat->childs.add( new btisenemyok );
            btcombat->childs.add( new btlookatenemy );

            btcanshoot *btfight = new btcanshoot;
            btfight->childs.add( new btcounterattack );
            btfight->childs.add( new btattack );

            btcombat->childs.add( btfight );

            gbt.childs.add( btcombat );
    ...
    {
        bool allowmove = canmove(d) && b.type != AI_S_WAIT && aienabled;
    ...
    void render()
    {
        if(aidebug > 1)
        {
            ...
            loopv(players) if(players[i]->state == CS_ALIVE && players[i]->ai)
            {
                ...
                if(aidebug >= 3)
                {
                    if(d->ai->weappref >= 0 && d->ai->weappref < 3)
                    {
                        particle_textcopy(pos, classes[ d->classid ].classname, PART_TEXT, 1);
                        pos.z += 2;
                    }
                    gameent *e = getclient(d->ai->enemy);
                    if(e)
            ...
            if(aidebug >= 4)
            {
                ...
                loopv(obstacles.obstacles)
                {
                    ...
                    for(; cur < next; cur++)
                    {
                        ...
                        if(iswaypoint(ent))
                            regular_particle_splash(PART_EDIT, 2, 40, waypoints[ent].o, 1.5f);
                    }
                    cur = next;
                }
            }
