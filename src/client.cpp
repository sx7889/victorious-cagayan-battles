...
namespace game
{
    ...
    static vector< timedevent* > timedevents;
    void addevent( timedevent *t ) {
        if( timedevents.empty() ) {
            timedevents.insert( 0, t );
            return;
        }

        loopvrev( timedevents ) {
            timedevent* ev = timedevents[ i ];
            if( t->millis > ev->millis ) {
                timedevents.insert( i+1, t );
                return;
            }
        }
    ...
    struct stunevent : timedevent {
        int cn;
        int delay;

        void process() {
            static const int extra_stun = - 300;
            gameent *target = getclient( cn );
            if( !target ) return;
            target->gunstarttime = 0;
            target->gunwait = 0;
            target->stuntime = millis + delay;
            target->laststun = millis;

            int lastskill = target->lastskill;
            if( lastskill != NO_SKILL ) {
                target->nextskill = NO_SKILL;
                target->lastaction = lastmillis;
                target->skilltrigger = false;
                target->reloadtime = lastmillis;
                target->lastfiredatk = lastskill;
                target->gunfires = 1;
            }

            if( target->ai ) {
                target->ai->guardtime = target->stuntime + ( delay + 200 );
            }
        }
    };

    struct chainevent : timedevent {
        int cn;

        void process() {
            gameent *target = getclient( cn );
            if( !target ) return;
            target->lastchaintime = lastmillis;
        }
    };

    struct knockbackevent : timedevent {
        int cn;
        vec vel;
    ...
    struct damageevent : timedevent {
        ...
        void process() {
            gameent *attacker = getclient( attackern );
            gameent *target = getclient( targetn );
            if( !target ) return;
            int diff = target->health - health;
            target->health = health;
            target->lastpain = lastmillis;

            if((target->state!=CS_ALIVE && target->state != CS_LAGGED && target->state != CS_SPAWNING) || intermission) return;

            if( m_singleplayer || diff != 0 ) {
                gameent *h = hudplayer();
                if(target==h) {
                    damageblend(10);
                    //damagecompass(10, actor->o);
                    playsound(S_PAIN2);
                }
                else if( attacker == h ) {
                    target->lastpainfromlocal = lastmillis;
                }
                damageeffect(10, target, target!=h);

                //ai::damaged(target, actor);

                //else playsound(S_PAIN1, &target->o);
                
            }
        }
    };

    struct diedevent : timedevent {
        int cn;
        int scn;
        int flags;
        ...
        void process() {
            gameent *target = getclient( cn );
            gameent *actor = getclient( scn );

            ++actor->frags;
            //if(m_teammode) setteaminfo(actor->team, tfrags);
            if(!target) return;
            target->cloakmode = 0;
            killed( target, actor, flags );
        }
    };
    ...
    {
        defformatstring(blipname, "media/interface/radar/blip%s%s.png", teams[validteam(team) ? team : 0].blipcolor, type);
    ...
    domiclientmode domimode;

    void setclientmode()
    {
        if(m_ctf) cmode = &ctfmode;
        else if(m_domination) cmode = &domimode;
        else cmode = NULL;
    }
    ...
    void printteam()
    {
        ...
        else conoutf("your team is: \fs%s%s\fr", teams[player1->team].textcode, teams[player1->team].name);
    }
    /*ICOMMAND(team, "sN", (char *s, int *numargs),
    {
        if(*numargs > 0) switchteam(s);
        else if(!*numargs) printteam();
        else if((player1->clientnum < 0 || m_teammode) && validteam(player1->team)) result(tempformatstring("\fs%s%s\fr", teams[player1->team].textcode, teams[player1->team].name));
    });*/
    ICOMMAND(getteam, "", (), intret((player1->clientnum < 0 || m_teammode) && validteam(player1->team) ? player1->team : 0));
    ICOMMAND(getteamname, "i", (int *num), result( tempformatstring( "\fs%s%s\fr", teams[ *num ].textcode, teamname(*num))));
    ...
    const char *getclienticon(int cn)
    {
        ...
        return m_teammode && validteam(d->team) ? teams[d->team].icon : teams[0].icon;
    }
    ...
    void setmode(int mode)
    {
        if(multiplayer(false) && !m_mp(mode))
        {
            conoutf(CON_ERROR, "mode %s (%d) not supported in multiplayer (setmode)",  server::modeprettyname(mode), mode);
            intret(0);
            return;
        }
        nextmode = mode;
    ...
    bool allowmouselook() {
        bool ret = !gamepaused || !remote || m_edit;
    ...
    void gamedisconnect(bool cleanup)
    {
        ...
        execident( "disconnected" );
    }

    void toserver(char *text) { conoutf(CON_CHAT, "%s:%s %s", colorname(player1), teams[0].textcode, text); addmsg(N_TEXT, "rcs", player1, text); }
    COMMANDN(say, toserver, "C");

    void sayteam(char *text) { if(!m_teammode || !validteam(player1->team)) return; conoutf(CON_TEAMCHAT, "%s:%s %s", colorname(player1), teams[player1->team].textcode, text); addmsg(N_SAYTEAM, "rcs", player1, text); }
    COMMAND(sayteam, "C");
    ...
    void parsepositions(ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            {
                int cn = getuint(p), physstate = p.get(), nextskill = getint(p), flags = getuint(p);
                vec o, vel, falling;
    ...
    void parsestate(gameent *d, ucharbuf &p, bool resume = false)
    {
        ...
        d->cloakmode = getint(p);
        d->grabentity = getint(p);
    }
    ...
    void parsemessages(int cn, gameent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) {
            type = getint( p );
            if( cmode && cmode->parseclientmessage( type, p, cmode ) ) {
                continue;
            }
            switch( type )
            {
                ...
                case N_SERVINFO:                   // welcome messsage from the server
                {
                    ...
                    {
                        if( PROTOCOL_VERSION < prot ) {
                            conoutf( CON_ERROR, "your client is older than the server, please update to latest version" );
                        }
                        disconnect();
                        return;
                    }
                ...
                case N_TEXT:
                {
                    ...
                    conoutf(CON_CHAT, "%s:%s %s", colorname(d), teams[0].textcode, text);
                    break;
                }
                ...
                case N_SAYTEAM:
                {
                    ...
                    if(t->state!=CS_DEAD && t->state!=CS_SPECTATOR)
                        particle_textcopy(t->abovehead(), text, PART_TEXT, 2000, teams[team].textcolor, 4.0f, -8);
                    conoutf(CON_TEAMCHAT, "%s:%s %s", colorname(t), teams[team].textcode, text);
                    break;
                }
                ...
                case N_ITEMLIST:
                {
                    ...
                    while((n = getint(p))>=0 && !p.overread())
                    {
                        ...
                        getint(p); // type
                        int grabbed = getint(p); //grab
                        float x = getint(p)/DMF; //x
                        float y = getint(p)/DMF; //y
                        float z = getint(p)/DMF; //z
                        int attr1 = getint(p); //grab
                        int attr2 = getint(p); //grab
                        int attr3 = getint(p); //grab
                        int attr4 = getint(p); //grab
                        int attr5 = getint(p); //grab

                        entities::update( n, grabbed == 1, x, y, z, attr1, attr2, attr3, attr4, attr5 );
                    }
                    break;
                }
                ...
                case N_SWITCHCLASS:
                {
                    int wn = getint(p), cls = getint(p);
                    gameent *w = getclient(wn);
                    if(!w) return;
                    w->classid = cls;
                    if(w->ragdoll) cleanragdoll(w);
                    break;
                }
                ...
                case N_KNOCKBACK:
                {
                    int tcn = getint(p), knockback = getint(p);
                    gameent *target = getclient(tcn);
                    vec dir;
                    loopk(3) dir[k] = getint(p)/DNF;
                    if( !target ) break;
                    vec kbv = vec(dir).mul( knockback );
                    target->vel.add( kbv );
                    break;
                }
                ...
                case N_CLIENTUPDATE: {
                    int scn = getint(p);
                    gameent *s = getclient(scn);
                    parsestate(s, p, false);
                    break;
                }

                case N_USE_ENTITY: {
                    int ecn = getint(p), ent = getint(p);
                    gameent *e = getclient(ecn);
                    if( e ) {
                        entities::effects( ent, e );
                    }
                    break;
                }
                ...
                case N_COMBAT:
                {
                    ...
                    vector< scstatus > hits;
                    int targetHits = getint( p );
                    loopi( targetHits ) {
                        scstatus &thd = hits.add();
                        thd.cn = getint( p );
                        thd.health = getint( p );
                        thd.flags = getint( p );
                    }
                    ...
                    gameent *s = getclient(scn);
                    if( !s || atk == NO_SKILL ) break;
                    int gun = 0;
                    int prevaction = s->lastaction;
                    int hitdelay = skills[atk].hitdelay;
                    int hitmillis = lastmillis + hitdelay;
                    int knockback = skills[atk].knockback;
                    bool forceknockback = false;
                    bool islocal = s == player1;

                    if( !islocal ) {
                        s->gunwait = skills[atk].attackdelay;
                        s->lastaction = lastmillis;
                        s->lastskill = atk;
                    }

                    shoteffects( atk, from, to, s, false, id, prevaction, flags );

                    if(m_teammode) {
                        setteaminfo( s->team, teamfrags );
                    }

                    if( ( flags & FL_HALF_DAMAGE ) ) {
                        if( islocal ) {
                            s->lasthalfdamagetime = lastmillis;
                        }
                    }

                    if( ( flags & FL_CHAIN ) ) {
                        s->lastchaintime = lastmillis;
                    }
                    else {
                        s->lastchaintime = 0;
                    }
                    ...
                    loopv( hits ) {
                        if( ( hits[i].flags & FL_KNOCKBACK ) ) {
                            vec dir = to;
                            dir.sub( from ).normalize();

                            knockbackevent* kbe = new knockbackevent;
                            kbe->cn = hits[i].cn;
                            kbe->vel = vec(dir).mul( min( 100, knockback ) );
                            kbe->millis = hitmillis;
                            addevent( kbe );
                        }

                        if( ( hits[i].flags & FL_STUN ) ) {
                            stunevent *se = new stunevent;
                            se->cn = hits[i].cn;
                            se->delay = STUNDELAY;
                            se->millis = hitmillis;
                        ...
                        if( ( hits[i].flags & FL_CHAIN ) ) {
                            chainevent *ce = new chainevent;
                            ce->cn = hits[i].cn;
                            ce->millis = hitmillis;
                            addevent( ce );
                        }

                        if( ( hits[i].flags & FL_BLOCKED ) || ( hits[i].flags & FL_PARRY ) ) {
                            blockevent *be = new blockevent;
                            be->cn = hits[i].cn;
                            be->flags = hits[i].flags;
                            be->millis = hitmillis;
                            addevent( be );
                        }
        
                        if( !( hits[i].flags & FL_NO_DAMAGE ) ) {
                            damageevent* de = new damageevent;
                            de->attackern = scn;
                            de->targetn = hits[i].cn;
                            de->health = hits[i].health;
                            de->flags = hits[i].flags;
                            de->millis = hitmillis;
                            addevent( de );
                        }

                        if( hits[i].health <= 0 ) {
                            diedevent* ded = new diedevent;
                            ded->cn = hits[i].cn;
                            ded->scn = scn;
                            ded->millis = hitmillis;
                            ded->flags = hits[i].flags;
                            addevent( ded );
                        }
                    }
                    ...
                    if( m_singleplayer ) {
                        sp_skill = atk;
                        sp_halfdamage = ( flags & FL_HALF_DAMAGE ) != 0 ? 1 : 0;
                        sp_hit = hits.length();
                        defformatstring( identname, "playerskill" );
                        execident( identname );
                    }
                    break;
                }
                ...
                case N_SETTEAM:
                {
                    ...
                    static const char * const fmt[2] = { "%s switched to %s", "%s forced to %s"};
                    if(reason >= 0 && size_t(reason) < sizeof(fmt)/sizeof(fmt[0]))
                        conoutf(fmt[reason], colorname(w), teams[w->team].name);
                    break;
                }
                ...
                case N_INITAI:
                {
                    ...
                    if(!b) break;
                    ai::init(b, at, on, sk, bn, 0, 0, name, team, classid);
                    break;
                }
    ...
    ICOMMAND(joinas, "i", ( int *cls ),
    {
        int classidx = *cls;
        if( classidx < 0
        || classidx >= NUMCLASSES ) {
            conoutf( "value should be equal or greater than 0 and lower than %d", NUMCLASSES );
            return;
        }

        const classinfo& cls = classes[ classidx ];
        int teamidx = cls.team;

        if( teamidx != player1->team ) {
            if(!validteam(teamidx)) return;
            if(player1->clientnum < 0) player1->team = teamidx;
            else addmsg(N_SWITCHTEAM, "ri", teamidx);
        }

        if( classidx != player1->classid ) {
            addmsg( N_SWITCHCLASS, "ri", classidx );
            suicide( player1 );
        }

        int i = player1->clientnum;
        if(i>=0) addmsg(N_SPECTATOR, "rii", i, 0);
    });

    void processevents() {
        while(!timedevents.empty())
        {
            timedevent *te = timedevents[0];
            if( te->millis < lastmillis ) {
                te->process();
                delete timedevents.remove(0);
            }
            else break;
        }
    }
