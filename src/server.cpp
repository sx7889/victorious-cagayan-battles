...
namespace game
{
    const char *gameident() { return "Victorious"; }
}
...
namespace server
{
    ...
    namespace aiman
    {
        ...
        extern void clearai();
        extern void checkai();
        extern void reqadd(clientinfo *ci, int skill, int team, int cls);
    ...
    int servmode::fragvalue(clientinfo *victim, clientinfo *actor)
    {
    ...
    VAR(friendlyfire, 0, 0, 1);
    VAR(capturereward, 0, 0, 1);
    VAR(daytime, 0, 0, 2);
    ...
    void setupdemoplayback()
    {
        ...
        {
            lilswap(&hdr.version, 2);
            if(hdr.version!=DEMO_VERSION) formatstring(msg, "demo \"%s\" requires an %s version of Victorious", file, hdr.version<DEMO_VERSION ? "older" : "newer");
            else if(hdr.protocol!=PROTOCOL_VERSION) formatstring(msg, "demo \"%s\" requires an %s version of Victorious", file, hdr.protocol<PROTOCOL_VERSION ? "older" : "newer");
        }
        if(msg[0])
        {
            DELETEP(demoplayback);
    ...
    } msgfilter(-1, N_CONNECT, N_SERVINFO, N_INITCLIENT, N_WELCOME, N_MAPCHANGE, N_SERVMSG, N_COMBAT, N_HITPUSH, N_DIED, N_SPAWNSTATE, N_FORCEDEATH, N_TEAMINFO, N_ITEMACC, N_ITEMSPAWN, N_TIMEUP, N_CDIS, N_CURRENTMASTER, N_PONG, N_RESUME, N_SENDDEMOLIST, N_SENDDEMO, N_DEMOPLAYBACK, N_SENDMAP, N_DROPFLAG, N_SCOREFLAG, N_RETURNFLAG, N_RESETFLAG, N_CLIENT, N_AUTHCHAL, N_INITAI, N_DEMOPACKET, -2, N_CALCLIGHT, N_REMIP, N_NEWMAP, N_GETMAP, N_SENDMAP, N_CLIPBOARD, -3, N_EDITENT, N_EDITF, N_EDITT, N_EDITM, N_FLIP, N_COPY, N_PASTE, N_ROTATE, N_REPLACE, N_DELCUBE, N_EDITVAR, N_EDITVSLOT, N_UNDO, N_REDO, -4, N_POS, NUMMSG),
    ...
    void sendstate( clientinfo* ci, T &p)
    {
        ...
        putint( p, ci->cloakmode );
        putint( p, gs.grabentity );
    }
    ...
    void sendspawn(clientinfo *ci)
    {
        servstate &gs = ci->state;
        spawnstate(ci);
        sendf(ci->ownernum, 1, "rii6", N_SPAWNSTATE, ci->clientnum, gs.lifesequence,
            gs.health, gs.maxhealth,
            ci->cloakmode, gs.grabentity);
    ...
    {
        servstate &gs = ci->state;
        sendf(-1, 1, "ri3i7ii", N_RESUME, ci->clientnum, gs.state,
    ...
    void loaditems()
    {
        ...
        loopv(ments) if(canspawnitem(ments[i].type))
        {
            ...
            sents[i].grabbed = 0;
            sents[i].x = ments[i].o.x*DMF;
            sents[i].y = ments[i].o.y*DMF;
            sents[i].z = ments[i].o.z*DMF;
            sents[i].attr1 = ments[i].attr1;
            sents[i].attr2 = ments[i].attr2;
            sents[i].attr3 = ments[i].attr3;
            sents[i].attr4 = ments[i].attr4;
            sents[i].attr5 = ments[i].attr5;
        }
        notgotitems = false;
    }
    ...
    void changemap(const char *s, int mode)
    {
        ...
        else if(m_domination) smode = &domimode;
        else smode = NULL;
    ...
    void skillevent::process(clientinfo *ci)
    {
        servstate &gs = ci->state;
        ...
        if( skills[atk].range && from.dist(to) > skills[atk].range + 1 )
            return;
        bool owned = skills[atk].owned;
        if( owned ) {
            if( wait<gs.gunwait ) {
                return;
            }
            gs.lastshot = millis;
            gs.gunwait = skills[atk].attackdelay;
            gs.nextskill = NO_SKILL;
            gs.lastskill = atk;
        }
        int knockback = skills[atk].knockback;
        bool canbeblocked = skills[atk].canbeblocked;
        bool breaksblock = skills[atk].breaksblock;
        int inflicteddamage = 0;
        int flags = 0;
        teaminfo *t = m_teammode && validteam(ci->team) ? &teaminfos[ci->team-1] : NULL;
        int teamfrags = 0;

        if( millis - gs.lastchain > CHAIN_COOLDOWN ) {
            gs.chainlevel = 0;
        }

        int skilldamage = skills[ atk ].damage;
        if( gs.chainlevel == 0 && owned && millis-gs.lastannounce < 500 ) {
            skilldamage = skilldamage  >> 1;
            flags |= FL_HALF_DAMAGE;
            breaksblock = false;
        }

        vector< damaged > damages;

        bool canchain = true;
        int totalrays = 0, maxrays = skills[atk].rays, totalhits = 0;
        loopv(hits)
        {
            hitinfo &h = hits[i];
            h.valid = false;

            bool addheadshot = ( h.flags & FL_HEADSHOT );
            h.flags = 0;
            if( addheadshot ) {
                h.flags = FL_HEADSHOT;
            }
            ...
            if( skills[atk].range && h.dist > skills[atk].range + 1) {
                continue;
            }

            if( !friendlyfire && isteam(target->team, ci->team) ) {
                continue;
            }

            totalrays += h.rays;
            if(totalrays>maxrays) continue;
            int damage = h.rays*skilldamage;
            servstate &ts = target->state;
            bool tknockback = knockback;

            int tatk = ts.nextskill;
            if( tatk != NO_SKILL ) {
                int parry = skills[tatk].parry
                // from attacker ,bleeding = skills[tatk].bleeding
                ,block = skills[tatk].block
                ,chains = skills[tatk].chains
                ;

                if( canbeblocked ) {
                    if( breaksblock && block ) {
                        h.flags |= FL_NO_DAMAGE | FL_STUN | FL_KNOCKBACK;
                        ts.nextskill = NO_SKILL;
                        ts.chainlevel = 0;
                        chains = false;
                        h.valid = true;
                        ++totalhits;
                    ...
                    if( parry ) {
                        ...
                        damage -= ( rnd( parry ) * damage ) / 100;
                        if( predamage != damage ) {
                            flags |= FL_STUN | FL_PARRY;
                            h.flags |= FL_PARRY;
                            if( chains ) {
                                ++ts.chainlevel;
                                ts.lastchain = millis;
            ...
            if( tknockback ) {
                h.flags |= FL_KNOCKBACK;
            }

            if( m_singleplayer && !sp_damage ) {
                damage = 0;
            }
            ts.dodamage( damage );
            ts.chainlevel = 0;
            ...
            if( ts.health <= 0 ) {
                target->state.deaths++;
                int fragvalue = smode ? smode->fragvalue(target, ci) : (target==ci || isteam(target->team, ci->team) ? -1 : 1);
                ci->state.frags += fragvalue;
                if(fragvalue>0)
                {
                    int friends = 0, enemies = 0; // note: friends also includes the fragger
                    if(m_teammode) loopv(clients) if(clients[i]->team != ci->team) enemies++; else friends++;
                    else { friends = 1; enemies = clients.length()-1; }
                    ci->state.effectiveness += fragvalue*friends/float(max(enemies, 1));
                }
                if(t) {
                    t->frags += fragvalue;
                }
                target->position.setsize(0);
                if(smode) smode->died(target, ci);
                ts.state = CS_DEAD;
                ts.lastdeath = gamemillis;
                if(ci!=target && m_teammode && ci->team == target->team)
                {
                    ci->state.teamkills++;
                    addteamkill(ci, target, 1);
                }
        ...
        gs.shotdamage += inflicteddamage;

        if( canchain && totalhits ) {
            bool chains = skills[ atk ].chains;
            if( chains ) { 
                ++gs.chainlevel;
                gs.lastchain = millis;
                flags |= FL_CHAIN;
        ...
        putint( p, int( totalhits ) );
        loopv(hits)
        {
            hitinfo &h = hits[i];
            if( !h.valid ) continue;
            clientinfo *target = getinfo(h.target);
            putint( p, target->clientnum );
            putint( p, target->state.health );
            putint( p, h.flags );
        }
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        while((curmsg = p.length()) < p.maxlen) {
            ...
            if( smode && smode->parseservermessage( type, p, ci, cq, smode ) ) {
                continue;
            }
            switch( type )
            {
                case N_POS:
                {
                    ...
                    if(cp)
                    {
                        ...
                        if( nextskill != cp->state.nextskill
                        && nextskill != NO_SKILL ) {
                            cp->state.lastannounce = gamemillis;
                        }
                        cp->state.nextskill = nextskill;
                    }
                    break;
                }
                ...
                case N_USE_ENTITY: {
                    clientinfo *target = getinfo( getint(p) );
                    int entnum = getint(p);
                    if( target ) {
                        sendf(-1, 1, "riii", N_USE_ENTITY, target->clientnum, entnum);
                    }
                    break;
                }
                ...
                case N_GRAB_ENTITY: {
                    ...
                    int entnum = getint(p);
                    if( target ) {
                        if( sents.inrange(entnum) ) {
                            if( !sents[entnum].grabbed ) {
                                if( cq ) cq->state.grabentity = entnum;
                                sendf(-1, 1, "riii", N_GRAB_ENTITY, target->clientnum, entnum);
                                sents[entnum].grabbed = 1;
                            }
                            else{
                                conoutf( "[N_GRAB_ENTITY] grabbed" );
                            }
                        }
                        else {
                            conoutf( "[N_GRAB_ENTITY] not in range" );
                        }
                    }
                    break;
                }
                ...
                case N_ITEMDROP: {
                    ...
                    if( cq ) cq->state.grabentity = -1;

                    sendf( -1, 1, "rii2ii5", N_ITEMDROP, cn, i, type, attr1, attr2, attr3, attr4, attr5);
                    break;
                }
                ...
                case N_USE_SKILL:
                {
                    ...
                    se->id = getint(p);
                    se->millis = cq ? cq->geteventmillis(gamemillis, se->id) : 0;
                    se->atk = getint(p);
                    loopk(3) se->from[k] = getint(p)/DMF;
                    loopk(3) se->to[k] = getint(p)/DMF;
                    int hits = getint(p);
                    loopk(hits)
                    {
                        if(p.overread()) break;
                        hitinfo &hit = se->hits.add();
                        hit.target = getint(p);
                ...
                case N_SAYTEAM:
                {
                    ...
                    if(isdedicatedserver() && cq) logoutf("%s <%s>: %s", colorname(cq), teams[cq->team].name, text);
                    break;
                }
                ...
                case N_SWITCHCLASS: {
                    ci->classid = getint(p);
                    sendf(-1, 1, "riii", N_SWITCHCLASS, sender, ci->classid);
                    break;
                }
                ...
                case N_SWITCH_CLOAK_MODE: {
                    int mode = getint(p);
                    ci->cloakmode = mode;
                    sendf(-1, 1, "riii", N_CLOAK_MODE, ci->clientnum, mode );
                    break;
                }
                ...
                case N_ADDBOT:
                {
                    ...
                    team = getint(p),
                    cls = getint(p);
                    aiman::reqadd(ci, skill, team, cls);
                    break;
                }
    ...
    const char *defaultmaster() { return "google.com"; }
