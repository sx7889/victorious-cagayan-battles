...
namespace aiman
{
    ...
    int lastchoosenclass = -1;
    VARN(serverbotlimit, botlimit, 0, 64, MAXBOTS);
    ...
    bool addai(int skill, int team, int cls, int limit)
    {
        ...
        int selteam = team != 0 ? team : m_teammode ? chooseteam() : 0;
        ...
        if( botnames[0] != '\0' ) {
            vector<char*> nn;
            explodelist( botnames, nn );
            char *name = nn[ rnd( nn.length() ) ];
        ...
        if( selteam == 0 ) {
            do {
                ci->classid = rnd( NUMCLASSES );
            } while( ci->classid == lastchoosenclass );
            lastchoosenclass = ci->classid;
        }
        else if( m_singleplayer ) {
            ci->classid = cls;
        }
        else {
            do {
                loopi( NUMCLASSES ) {
                    const classinfo& clsi = classes[ i ];
                    if( clsi.team == selteam ) {
                        int dice = rnd( 100 );
                        if( dice > 50 ) {
                            ci->classid = i;
                            break;
    ...
    void reinitai(clientinfo *ci)
    {
        ...
        {
            sendf(-1, 1, "ri7s", N_INITAI, ci->clientnum, ci->ownernum, ci->state.aitype, ci->state.skill, ci->team, ci->classid, ci->name);
            if(ci->aireinit == 2)
            {
                ci->reassign();
    ...
    void reqadd(clientinfo *ci, int skill, int team, int cls)
    {
        if(!ci->local && !ci->privilege) { 
            return;
        }
        if(!addai(skill, team, cls, !ci->local && ci->privilege < PRIV_ADMIN ? botlimit : -1)) { 
