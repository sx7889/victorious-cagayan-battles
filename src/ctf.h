...
#endif
{
    ...
    bool parseservermessage( int type, packetbuf &p, server::clientinfo *ci, server::clientinfo *cq, server::servmode *smode ) {
        ctfservmode *ctfmode = static_cast< ctfservmode* >( smode );
        switch( type ) {
            case N_TRYDROPFLAG: {
    ...
    void drawhud(gameent *d, int w, int h)
    {
        if(d->state == CS_ALIVE)
        {
            ...
            {
                float x = HICON_SIZE/2, y = 1800*0.9f-HICON_SIZE/2;
    ...
    void rendergame()
    {
        loopv(flags)
        {
            ...
            const char *flagname = models[ teams[ f.team ].worldflag ].name;
            float angle;
    ...
    bool parseclientmessage( int type, ucharbuf &p, clientmode *m ) {
        ctfclientmode *ctfmode = static_cast< ctfclientmode* >( m );
        switch( type ) {
            case N_INITFLAGS:
            {
