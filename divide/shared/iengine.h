...
extern bool usecustomcamera;
extern vec cameraorigin;
extern vec cameraoffset;
...
extern void fillcolorquad( float x, float y, float w, float h, int rgb );
...
extern void regular_particle_splash(int type, int num, int fade, const vec &p, float size = 1.0f, int radius = 150, int gravity = 2, int delay = 0);
...
extern void particle_splash(int type, int num, int fade, const vec &p, float size = 1.0f, int radius = 150, int gravity = 2);
...
// worldio
extern const char *getworldname();
...
// rendermodel
enum { MDL_CULL_VFC = 1<<0, MDL_CULL_DIST = 1<<1, MDL_CULL_OCCLUDED = 1<<2, MDL_CULL_QUERY = 1<<3, MDL_FULLBRIGHT = 1<<4, MDL_NORENDER = 1<<5, MDL_MAPMODEL = 1<<6, MDL_NOBATCH = 1<<7, MDL_ONLYSHADOW = 1<<8, MDL_NOSHADOW = 1<<9, MDL_FORCESHADOW = 1<<10, MDL_FORCETRANSPARENT = 1<<11, MDL_WIREFRAME = 1<<12 };
...
namespace httpreq {
    extern bool start( const char* url, int port, int type, const char *fmt, ... );
    extern int getresponse( vector<char> &response );
    extern int update( vector<char> &response );
}

// sound
extern void resetsound();
extern void registersound(char *, int *);
