...
void fixwd( char **argv ) {
    char *cwd = getcwd( 0, 0 );
    char path[512], path2[512];
    formatstring( path, "%s%cconfig", cwd, kPathSeparator );

    if( !dirExists( path ) ) {
        char *pos = strrchr( cwd, kPathSeparator );
        if( pos ) {
            int cnt = pos - cwd;
            strncpy( path, cwd, cnt );
            path[ cnt ] = '\0';

            formatstring( path2, "%s%cconfig", path, kPathSeparator );

            if( !dirExists( path2 ) ) {
                fatal( "couldn't find config folder" );
            }
            ...
            fatal( "couldn't find root folder" );
        }
    }
}
