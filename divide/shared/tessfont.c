...
int main(int argc, char **argv)
{
    ...
    if( argc == 3 ) {
        ...
        sw = 32;
        sh = 64;
        texdir = "media/interface/font/";
    }
    else if( argc < 11 ) {
        fatal("Usage: tessfont infile outfile outborder[:inborder] pad offset advance charwidth charheight texwidth texheight [spacewidth spaceheight texdir]");
    }
    else {
    ...
    bool use_kerning = FT_HAS_KERNING( f );
    ...
    if(sw <= 0)
    {
        if(FT_Load_Char(f, ' ', FT_LOAD_DEFAULT))
            fatal("tessfont: failed loading space character");
    ...
    FT_Stroker_Done(s2);
    FT_Done_FreeType(l);
    printf("tessfont: (%d, %d) .. (%d, %d) = (%d, %d) / (%d, %d), %d texs\n", x1, y1, x2, y2, x2 - x1, y2 - y1, w2, h2, numtex);
    return EXIT_SUCCESS;
}
