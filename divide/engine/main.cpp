...
void fatal(const char *s, ...)    // failure exit
{
    ...
    if(errors <= 2) // print up to one extra recursive error
    {
        ...
        if(errors <= 1) // avoid recursion
        {
            ...
            SDL_Quit();
            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "divide fatal error", msg, NULL);
        }
    }
...
void fillcolorquad( float x, float y, float w, float h, int rgb ) {
    int r = (rgb >> 16) & 0xFF;
    int g = (rgb >> 8) & 0xFF;
    int b = rgb & 0xFF;
...
void renderbackgroundview(int w, int h, const char *caption, const char *info)
{
    ...
    settexture("media/interface/title/background.png", 0);
    bgquad(0, 0, w, h);
    ...
    if(caption)
    {
        ...
        float tsz = 0.05f*min(w, h)/FONTH,
              tx = 0.5f*(w - tw*tsz), ty = 0.8f*min(w, h);
        pushhudtranslate(tx, ty, tsz);
        draw_text(caption, 0, 0);
        pophudmatrix();
    }
    if( info ) {
        int tw = text_width(info);
        float tsz = 0.04f*min(w, h)/FONTH,
              tx = 0.5f*(w - tw*tsz), ty = 0.85f*min(w, h);
        pushhudtranslate(tx, ty, tsz);
        draw_text(info, 0, 0);
        pophudmatrix();
    }
...
void renderbackground(const char *caption, const char *info, bool force)
{
    ...
    if(force)
    {
        renderbackgroundview(w, h, caption, info);
        return;
    }
...
void restorebackground(int w, int h, bool force = false)
{
    ...
    renderbackgroundview(w, h, backgroundcaption[0] ? backgroundcaption : NULL, backgroundmapinfo);
}
...
void setupscreen()
{
    ...
    screen = SDL_CreateWindow("Victorious", winx, winy, winw, winh, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_INPUT_FOCUS | SDL_WINDOW_MOUSE_FOCUS | flags);
...
void resetgl()
{
    ...
    if(!reloadtexture(*notexture) ||
       !reloadtexture("media/interface/title/background.png") ||
...
void stackdumper(unsigned int type, EXCEPTION_POINTERS *ep)
{
    ...
    char out[512];
    formatstring(out, "Victorious Win32 Exception: 0x%x [0x%x]\n\n", er->ExceptionCode, er->ExceptionCode==EXCEPTION_ACCESS_VIOLATION ? er->ExceptionInformation[1] : -1);
...
int main(int argc, char **argv)
{
    ...
    loopv( gameargs ) {
        if( !game::clientoption( gameargs[i] ) ) {
