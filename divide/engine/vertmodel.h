...
template<class MDL> struct vertcommands : modelcommands<MDL, struct MDL::vertmesh>
{
    ...
    {
        if(!MDL::loading) { conoutf("not loading an %s (%s)", MDL::formatname(), model); return; }
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (tag:%s)", MDL::formatname(), tagname); return; }
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (pitch)", MDL::formatname()); return; }
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (anim:%s)", MDL::formatname(), anim); return; }
        vector<int> anims;
