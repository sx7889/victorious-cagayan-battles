...
static bool loadmapheader(stream *f, const char *ogzname, mapheader &hdr, octaheader &ohdr)
{
    ...
    {
        if(hdr.version>MAPVERSION) { conoutf(CON_ERROR, "map %s requires a newer version of Victorious", ogzname); return false; }
...
#ifndef STANDALONE
string ogzname, bakname, cfgname;
string worldname = "";
...
bool save_world(const char *mname, bool nolms)
{
    ...
    {
        if((id.type == ID_VAR || id.type == ID_FVAR || id.type == ID_SVAR) && id.flags&IDF_OVERRIDE && !(id.flags&IDF_READONLY) && id.flags&IDF_OVERRIDDEN && !(id.flags&IDF_PRIVATE)) hdr.numvars++;
    });
    ...
    enumerate(idents, ident, id,
    {
        if((id.type!=ID_VAR && id.type!=ID_FVAR && id.type!=ID_SVAR) || !(id.flags&IDF_OVERRIDE) || id.flags&IDF_READONLY || !(id.flags&IDF_OVERRIDDEN) || id.flags&IDF_PRIVATE) continue;
        f->putchar(id.type);
...
void save_worldconfig( const char *mname ) {
    setmapfilenames(*mname ? mname : "untitled");
    ...
    if( f ) {
        ...
        loopv(vslots) {
            ...
            if( slot.shader ) {
                ...
                loopvj( slot.params ) {
                    SlotShaderParam &p = slot.params[j];
                    fprintf( f, "setshaderparam %s %f %f %f %f\n", p.name, p.val[0], p.val[1], p.val[2], p.val[3] );
                }
            }
            loopvj(slot.sts) {
                Slot::Tex &c = slot.sts[j];
                fprintf( f, "texture %c \"%s\"", findslotname( j ), c.name );
                if( j == 0 ) {
                    fprintf( f, " %d %d %d %f // %d", vs.rotation, vs.offset.x, vs.offset.y, vs.scale, i );
                }
                fprintf( f, "\n" );
            }
            fprintf( f, "\n" );
...
bool load_world( const char *mname, const char *cname )        // still supports all map formats that have existed since the earliest cube betas!
{
    ...
    renderbackground( "loading...", game::getmapinfo() );
...
void createcurrentmapconfig() { save_worldconfig(game::getclientmap()); }
COMMAND( createcurrentmapconfig, "" );
