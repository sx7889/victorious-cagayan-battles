...
struct animmodel : model
{
    ...
    struct skin : shaderparams
    {
        ...
        void setshaderparams(mesh &m, const animstate *as, bool skinned = true)
        {
            ...
            LOCALPARAMF(maskvalues, metal, roughness, ao);
    ...
    struct part
    {
        ...
        void render(int anim, int basetime, int basetime2, float pitch, const vec &axis, const vec &forward, dynent *d, animstate *as)
        {
            ...
            if(!(anim&ANIM_NORENDER))
            {
                ...
                GLOBALPARAM( sundir, sunlightdir );

                float lightscale = 2.0f*ldrscaleb;
                GLOBALPARAMF( ambient, ambient.x*lightscale*ambientscale, ambient.y*lightscale*ambientscale, ambient.z*lightscale*ambientscale, 255*lightscale);
            }
    ...
    void setmetal(float val) {
        if(parts.empty()) loaddefaultparts();
        loopv(parts) loopvj(parts[i]->skins) parts[i]->skins[j].metal = val;
    }

    void setroughness(float val) {
        if(parts.empty()) loaddefaultparts();
        loopv(parts) loopvj(parts[i]->skins) parts[i]->skins[j].roughness = val;
    }

    void setao(float val) {
        if(parts.empty()) loaddefaultparts();
        loopv(parts) loopvj(parts[i]->skins) parts[i]->skins[j].ao = val;
    }
...
template<class MDL, class MESH> struct modelcommands
{
    ...
    {
        if(!MDL::loading) { conoutf("not loading an %s (%s)", MDL::formatname(), name); return; }
        formatstring(MDL::dir, "media/model/%s", name);
    }

    #define loopmeshes(meshname, m, body) do { \
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (mesh:%s)", MDL::formatname(), meshname); return; } \
    ...
    static void setmetal(char *meshname, float *val) {
        loopskins(meshname, s, s.metal = *val);
    }

    static void setroughness(char *meshname, float *val) {
        loopskins(meshname, s, s.roughness = *val);
    }

    static void setao(char *meshname, float *val) {
        loopskins(meshname, s, s.ao = *val);
    }
    ...
    {
        if(!MDL::loading) { conoutf("not loading an %s (tag %s)", MDL::formatname(), tagname); return; }
    ...
    modelcommands()
    {
        ...
        if(MDL::multimeshed())
        {
            ...
            modelcommand(setmetal, "metal", "sf");
            modelcommand(setroughness, "roughness", "sf");
            modelcommand(setao, "ao", "sf");
        }
