...
struct model
{
    ...
    virtual ~model() { loddestroy(); DELETEA(name); DELETEP(bih); }
    ...
    virtual void setmetal( float ) {}
    virtual void setroughness( float ) {}
    virtual void setao( float ) {}
    ...
    void addlod( const char *name, float dist ) {
        lodlevel *ll = new lodlevel;
        ll->name = newstring(name);
        ll->dist = dist;
