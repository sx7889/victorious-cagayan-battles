...
#ifdef WIN32
    MessageBox(NULL, msg, "Victorious fatal error", MB_OK|MB_SYSTEMMODAL);
#else
...
void disconnect_client(int n, int reason)
{
    ...
    string s;
    if(msg) formatstring(s, "client (%s) disconnected because: %s (%d)", clients[n]->hostname, msg, reason);
    else formatstring(s, "client (%s) disconnected (%d)", clients[n]->hostname, reason);
    logoutf("%s", s);
...
static bool sresolverwait(const char *name, ENetAddress *address)
{
...
static void rendermsg( const char *msg) {
#ifndef STANDALONE
    renderprogress(0, msg );
#endif
}
...
namespace httpreq {
    ...
    bool start( const char* url, int port, int type, const char *fmt, ... ) {
        if(sock != ENET_SOCKET_NULL) {
            logoutf( "[httpreq] another request is in progress..." );
            return false;
        }
        ...
        if(sock == ENET_SOCKET_NULL)
        {
            logoutf( "[httpreq] could not open socket" );
            return false;
        }
        ...
        if(!sresolverwait(url, &addr)) {
            logoutf( "[httpreq] couldn't resolve %s...", url );
            return false;
        }
        
        if( enet_socket_connect(sock, &addr) ) {
            enet_socket_destroy(sock);
            logoutf( "[httpreq] could not connect to server %s...", url );
            return false;
        }

        defvformatstring( params, fmt, fmt );

        static char req[1024] = "";
        if( type == HTTP_GET ) {
            sprintf( req,
                "GET /services.php%s%s HTTP/1.1\r\n"
                "Host: %s\r\n"
                "User-Agent: Any\r\n"
                "\r\n",
                params[0] ? "?":"",
        ...
        rendermsg( "sending request..." );

        ENetBuffer buf;
        buf.data = &req[0];
        buf.dataLength = strlen(req);
        int sent = enet_socket_send(sock, NULL, &buf, 1);

        if( sent < 0 ) {
            enet_socket_destroy(sock);
            sock = ENET_SOCKET_NULL;
            logoutf( "[httpreq] error while sending (%d)", sent );
            return false;
        }
    ...
    int getresponse( vector<char> &response ) {
        if( sock == ENET_SOCKET_NULL ) { 
            return 0;
        }
        vector<char> in;
        ...
        enet_socket_wait( sock, &events, 2000 );
        if( !(events & ENET_SOCKET_WAIT_RECEIVE) ) {
            return -1;
        }
        ...
        if(recv > 0) {
            ...
            logoutf( "[httpreq::getresponse] errno: %d", recv );
            enet_socket_destroy( sock );
            sock = ENET_SOCKET_NULL;
            return 0;
        }
        ...
        int ret = 0;
        if( strstr( input, "HTTP/1.1" ) ) {
            sscanf( input, "HTTP/1.1 %d", &ret );
        }

        char *start = strstr( input, "\r\n\r\n" );
        if( start ) {
            start += 4;
            int len = strlen( input ) - (start-input);
            response.reserve( len + 1 );
            memcpy( response.getbuf(), start, len );
            response.advance( len );
            response[len] = '\0';
        }
    ...
    int update( vector<char> &response ) {
        static ENetSocketSet readset, writeset;
        ENET_SOCKETSET_EMPTY(readset);
        ENET_SOCKETSET_EMPTY(writeset);
        ...
        if( sock == ENET_SOCKET_NULL || enet_socketset_select( sock, &readset, &writeset, 10 ) <= 0 ) {
            return 0;
        }
...
{
    static vector< char > resp;
    resp.shrink( 0 );
    httpreq::update( resp );

    if( lastupdatemaster <= totalmillis ) {
        if(mastername[0] && allowupdatemaster) {
            logoutf( "registering with master..." );
            if( httpreq::start( mastername, masterport, HTTP_POST, "port=%d", serverport ) ) {
                lastupdatemaster = totalmillis + 5 * 60 * 1000;
            }
            else {
                logoutf( "retring in 1 minute..." );
                lastupdatemaster = totalmillis + 1 * 60 * 1000;
            }
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    loopv( gameargs ) {
        if( !server::serveroption( gameargs[i] ) ) {
            logoutf( "unknow option %s", gameargs[i] );
        }
    }
