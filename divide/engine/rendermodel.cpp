...
void mdllod( const char *name, float *dist ) {
    checkmdl;
...
model *findlod( model *m, const vec &pos ) {
    if( !m ) {
        return 0;
    }
...
void rendermapmodel(int idx, int anim, const vec &o, float yaw, float pitch, float roll, int flags, int basetime, float size)
{
    ...
    if(shadowmapping)
    {
        ...
        if( cullmodel( m, center, radius, MDL_CULL_DIST ) ) {
            return;
        }
    } 
    else if( cullmodel(m, center, radius, flags) ) {
        return;
    }
