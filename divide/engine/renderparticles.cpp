...
static partrenderer *parts[] =
{
    ...
    new taperenderer("media/particle/spark.png", PT_TAPE|PT_FEW|PT_BRIGHT),               // rail trail
    new taperenderer("media/particle/spark.png", PT_TAPE|PT_FEW|PT_BRIGHT),               // pulse side
    new quadrenderer("media/particle/spark.png", PT_PART|PT_FLIP|PT_FEW|PT_BRIGHT),      // pulse front
    ...
    new quadrenderer("media/particle/spark.png", PT_PART|PT_FEW|PT_FLIP|PT_BRIGHT|PT_TRACK),  // rail muzzle flash
    new quadrenderer("media/particle/spark.png", PT_PART|PT_FEW|PT_FLIP|PT_BRIGHT|PT_TRACK), // pulse muzzle flash
    new quadrenderer("media/interface/hud/items.png", PT_PART|PT_FEW|PT_ICON|PT_LERP),                 // hud icon
    new quadrenderer("<colorify:1/1/1>media/interface/hud/items.png", PT_PART|PT_FEW|PT_ICON|PT_LERP), // grey hud icon
    &texts,                                                                                    // text
    &meters,                                                                                   // meter
    &metervs,                                                                                  // meter vs.
...
static void splash(int type, int radius, int num, int fade, const vec &p, float size, int gravity)
{
    ...
    loopi(num)
    {
        ...
        int color = game::particlecolor( type );
        newparticle(p, tmp, f, type, color, size, gravity)->val = collidez;
    }
}

static void regularsplash(int type, int radius, int num, int fade, const vec &p, float size, int gravity, int delay = 0)
{
    if(!canemitparticles() || (delay > 0 && rnd(delay) != 0)) return;
    splash(type, radius, num, fade, p, size, gravity);
}
...
void regular_particle_splash(int type, int num, int fade, const vec &p, float size, int radius, int gravity, int delay)
{
    if(!canaddparticles()) return;
    regularsplash(type, radius, num, fade, p, size, gravity, delay);
}

void particle_splash(int type, int num, int fade, const vec &p, float size, int radius, int gravity)
{
    if(!canaddparticles()) return;
    splash(type, radius, num, fade, p, size, gravity);
}
...
static void makeparticles(entity &e)
{
    switch(e.attr1)
    {
        ...
        case 1: //steam vent - <dir>
            regularsplash(PART_STEAM, 50, 1, 200, offsetvec(e.o, e.attr2, rnd(10)), 2.4f, -20);
            break;
        case 2: //water fountain - <dir>
        {
            ...
            regularsplash(PART_WATER, 150, 4, 200, offsetvec(e.o, e.attr2, rnd(10)), 0.6f, 2);
            break;
        }
...
void updateparticles()
{
    ...
    if(editmode) // show sparkly thingies for map entities in edit mode
    {
        ...
        loopv(ents)
        {
            ...
            regular_particle_splash(PART_EDIT, 2, 40, e.o, 0.32f*particlesize/100.0f);
        }
    }
}
