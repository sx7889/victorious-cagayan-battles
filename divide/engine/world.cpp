    ...
    switch(e.type)
    {
        ...
        default:
            if(e.type >= ET_GAMESPECIFIC) {
                bool msg;
                if(model *m = loadmodel( entities::entmodel( e ), msg ) )
                {
                    vec center, radius;
                    mmboundbox(e, m, center, radius);
                    center.add(e.o);
...
bool dropentity(entity &e, int drop = -1)
{
    ...
    if(e.type == ET_MAPMODEL)
    {
        ...
        bool msg;
        model *m = loadmodel( entities::entmodel( e ), msg );
        if(m)
        {
            vec center;
            mmboundbox(e, m, center, radius);
