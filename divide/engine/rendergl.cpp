...
void printtimers(int conw, int conh, bool con)
{
    if( con && !frametimer && !usetimers) return;
    ...
    if(frametimer)
    {
        ...
        if( con )draw_textf("frame time %i ms", conw-20*FONTH, conh-FONTH*3/2-offset*9*FONTH/8, printmillis);
        else conoutf( "frame time %i ms", printmillis );
        offset++;
    }
    if(usetimers) loopv(timerorder)
    {
        ...
        if( con ) draw_textf("%s%s %5.2f ms", conw-20*FONTH, conh-FONTH*3/2-offset*9*FONTH/8, t.name, t.gpu ? "" : " (cpu)", t.print);
        else conoutf( "%s%s %5.2f ms",t.name, t.gpu ? "" : " (cpu)", t.print );
        offset++;
    }
...
{
    if(!game::allowmouselook() || mainmenu) return;
...
{
    static physent tempcustomcamera;
    ...
    if( usecustomcamera && thirdperson == 0 ) {
        camera1 = &tempcustomcamera;
        camera1->o = cameraorigin;
        camera1->o.add( cameraoffset );
    ...
    {
        camera1 = player;
        tempcustomcamera.yaw = player->yaw;
...
{
    renderbackground(NULL, NULL, true);
}
...
void gl_drawframe()
{
    ...
    vieww = hudw;
    viewh = hudh;
    if( getworldname()[0] == '\0' && mainmenu ) { gl_drawmainmenu(); }
    else gl_drawview();
    if( backgroundimage[0] ) gl_drawmainmenu();
    gl_drawhud();
    UI::render();
    gl_drawpostui();
