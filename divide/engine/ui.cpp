...
namespace UI
{
    ...
    struct Gradient2 : FillColor
    {
        ...
        void setup(int type_, int dir_, const Color &color_, const Color &color2_, float minw_ = 0, float minh_ = 0)
        {
            FillColor::setup(type_, color_, 0.0f, 0.0f);
            mw = minw_;
            mh = minh_;
            dir = dir_;
            color2 = color2_;
        ...
        void draw(float sx, float sy)
        {
            ...
            gle::begin(GL_TRIANGLE_STRIP);
            gle::attribf(mw*2, -mh);   (dir == HORIZONTAL ? color2 : color).attrib();
            gle::attribf(-mw,   -mh);   color.attrib();
            gle::attribf(mw*2, mh); color2.attrib();
            gle::attribf(-mw,   mh); (dir == HORIZONTAL ? color : color2).attrib();
            gle::end();
    ...
    static const char *getsval(ident *id, bool &shouldfree, const char *val = "")
    {
        switch(id->type)
        {
            case ID_VAR: {
                if( (id->flags & IDF_HEX ) ) {
                    static string b;
                    nformatstring( b, 20, "0x%.*X", max(6, 1), *id->storage.i);
                    val = b;
                }
                else {
    ...
    struct PlayerPreview : Preview
    {
        ...
        void setup(int classindex, float minw_, float minh_)
        {
            Preview::setup(minw_, minh_);
            classid = classindex;
        }
        ...
        void draw(float sx, float sy)
        {
            ...
            window->calcscissor(sx, sy, sx+w, sy+h, sx1, sy1, sx2, sy2, false);
            modelpreview::start(sx1, sy1, sx2-sx1, sy2-sy1, false, clipstack.length() > 0);
            game::renderplayerpreview( classid );
            if(clipstack.length()) clipstack.last().scissor();
            modelpreview::end();
        }
    };
    ...
    ICOMMAND(uimodvgradient2, "iiffe", (int *c, int *c2, float *minw, float *minh, uint *children),
        BUILD(Gradient2, o, o->setup(Gradient2::MODULATE, Gradient2::VERTICAL, Color(*c), Color(*c2), *minw, *minh), children));
    ...
    ICOMMAND(uiplayerpreview, "iffe", (int *classindex, float *minw, float *minh, uint *children),
        BUILD(PlayerPreview, o, o->setup(*classindex, *minw, *minh), children));
