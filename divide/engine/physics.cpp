...
bool mmcollide(physent *d, const vec &dir, float cutoff, octaentities &oc) // collide with a mapmodel
{
    ...
    loopv(oc.mapmodels)
    {
        ...
        if( e.type >= ET_GAMESPECIFIC ) {
            ...
            bool msg;
            m = loadmodel( entities::entmodel( e ), msg );
            if( !m ) {
                continue;
            }
            mcol = m->collide;
...
bool moveplayer(physent *pl, int moveres, bool local, int curtime)
{
    ...
    bool floating = pl->type==ENT_PLAYER && ( (pl->state==CS_EDITING || pl->state==CS_SPECTATOR) || pl->onladder );
...
#define dir(name,v,d,s,os,id) ICOMMAND(name, "D", \
    (int *down), { \
        if( mainmenu != 0 ) return; \
        player->s = *down!=0; \
        if( player->s ) { \
            if( game::allowmove( player ) ) { \
                if( player->k_doubletapid == id \
                && lastmillis - player->k_doubletaplast < 200 ) { \
                    game::onplayerdoubletap( player ); \
                    player->k_doubletaplast = lastmillis; \
                    player->k_doubletapid = -1; \
                } \
                else { \
                    player->k_doubletaplast = lastmillis; \
                    player->k_doubletapid = id; \
                } \
            } \
        } \
...
dir(backward, move,   -1, k_down,  k_up,    1);
dir(forward,  move,    1, k_up,    k_down,  2);
dir(left,     strafe,  1, k_left,  k_right, 3);
dir(right,    strafe, -1, k_right, k_left,  4);
