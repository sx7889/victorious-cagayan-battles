...
template<class MDL> struct skelcommands : modelcommands<MDL, struct MDL::skelmesh>
{
    ...
    {
        if(!MDL::loading) { conoutf("not loading an %s (mesh:%s skel:%s)", MDL::formatname(), meshfile, skelname); return; }
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (name:%s tag:%s)", MDL::formatname(), name, tagname); return; }
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (%s)", MDL::formatname(), name); return; }
        part &mdl = *(part *)MDL::loading->parts.last();
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("\frnot loading an %s (%s)", MDL::formatname(), name); return; }
        part &mdl = *(part *)MDL::loading->parts.last();
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("\frnot loading an %s (name:%s target:%s)", MDL::formatname(), name, targetname); return; }
        part &mdl = *(part *)MDL::loading->parts.last();
        if(!mdl.meshes) return;
        skeleton *skel = ((meshgroup *)mdl.meshes)->skel;
        int bone = skel ? skel->findbone(name) : -1;
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (anim:%s, animfile:%s)", MDL::formatname(), anim, animfile); return; }
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (mask:%s)", MDL::formatname(), maskstr); return; }
    ...
    {
        if(!MDL::loading || MDL::loading->parts.empty()) { conoutf("not loading an %s (%s)", MDL::formatname(), maskstr); return; }
