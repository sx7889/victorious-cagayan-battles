...
void retrieveservers(vector<char> &data)
{
    ...
    renderprogress(0, "retrieving servers from master..." );
    if( httpreq::start( mastername, masterport, HTTP_GET, "list=0" ) ) {
        int status = httpreq::getresponse( data );
        if( status == 200 ) {
            renderprogress( 1.0f, "done" );
        }
        else {
            defformatstring( text, "error %d", status );
            renderprogress( 1.0f, text );
        }
    }
}
