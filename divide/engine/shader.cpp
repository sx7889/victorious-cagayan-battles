...
struct postfxpass
{
    ...
    postfxpass() : shader(NULL), inputs(1), freeinputs(1), outputbind(0), outputscale(0), binddepth( false ) {}
};
...
void renderpostfx(GLuint outfbo)
{
    ...
    loopv(postfxpasses)
    {
        ...
        if( p.binddepth ) {
            ...
            glBindTexture( GL_TEXTURE_RECTANGLE, postfxtexs[ postfxbinds[0] ].id );
        ...
        GLOBALPARAMF( postfx, float(tw), float(th), 0.0f );
        screenquad(tw, th);
...
static bool addpostfx(const char *name, bool binddepth, int outputbind, int outputscale, uint inputs, uint freeinputs, const vec4 &params)
{
...
ICOMMAND(addpostfx, "siiisffff", (char *name, int *binddepth, int *bind, int *scale, char *inputs, float *x, float *y, float *z, float *w),
{
    ...
    bool bd = *binddepth == 0 ? false : true;
    addpostfx(name, bd, clamp(*bind, 0, NUMPOSTFXBINDS-1), max(*scale, 0), inputmask, freemask, vec4(*x, *y, *z, *w));
});

ICOMMAND(removepostfx, "s", (char *name), {
    Shader *s = useshaderbyname(name);
    if( !s ) {
        return;
    }
...
{
    clearpostfx();
    if(name[0]) addpostfx(name, false, 0, 0, 1, 1, vec4(*x, *y, *z, *w));
});
